    ...
    onFirstConnect() {
        ...
        this.styles.add({
            ...
            ".porthole-anchor": {
                ...
                transform: "scale(var(--scale)) rotateY(calc(var(--yaw) * 1deg)) rotateX(4deg) translateZ(65px)"
            },
            ".porthole": {
                position: "absolute",
...
if (!stage || !rocket) {
    console.warn("[playground:sprite-extended] Missing required DOM nodes for sprite-extended runtime.");
} else {
    let bound = false;
    const state = {
        amplitude: 60,
    ...
    const rocketAnimator = {
        ...
        update(time) {
            ...
            state.yaw = Math.sin(seconds * state.speed) * state.amplitude;
            rocket.setFrameByDegree(state.yaw);
        },
        render() {}
    };
    ...
    function bindRuntime() {
        ...
        rocket.auto = false;
        rocket.paused = true;
        stage.addAnimator(rocketAnimator);
        stage.timeline?.play?.();
        bound = true;
        return true;
    }
    ...
    Promise.all([customElements.whenDefined("animation-stage"), customElements.whenDefined("rocket-sprite")]).then(
        () => {
            bindWhenReady();
