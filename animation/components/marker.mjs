...
class Marker extends Component.HTMLElement {
    ...
    updatePosition() {
        ...
        ["x", "y"].forEach((axis) => {
            ...
            if (verticalPostions.includes(value)) {
                this.styles.update(`:host`, { top: value }, "position");
            } else if (horizontalPostions.includes(value)) {
                this.styles.update(`:host`, { left: value }, "position");
            } else if (units.some((unit) => String(value).endsWith(unit))) {
                this.styles.update(`:host`, { [isVertical ? "top" : "left"]: value }, "position");
            } else {
                this.styles.update(`:host`, { [isVertical ? "top" : "left"]: `${value}px` }, "position");
            }
        });
    }
