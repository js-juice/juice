...
class TransformFeedback {
    ...
    applyDownStreamData() {
        ...
        Object.keys(this._variables).forEach((name) => {
            ...
            const { gl, settings } = feedbackVar;
            if (feedbackVar.children.length > 0) {
                const location = feedbackVar.children[0].location;
                gl.bindBuffer(gl.ARRAY_BUFFER, feedbackVar.buffer.read);
    ...
    update(deltaTime) {
        ...
        // Important: unbind capture buffers so render pass can safely bind particle buffers as vertex inputs.
        Object.keys(this._variables).forEach((name) => {
            const variable = this._variables[name];
            const bindingIndex = Number.isFinite(variable.index) ? variable.index : 0;
            gl.bindBufferBase(gl.TRANSFORM_FEEDBACK_BUFFER, bindingIndex, null);
        });

        // Do not leak TF VAO into the render pass.
