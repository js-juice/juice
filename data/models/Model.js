...
class Model extends EventEmitter {
    ...
    saveRelated() {
        ...
        return this.async ? Promise.all(saving) : saving;
    }
    ...
    changes() {
        const schema = this.static.schema;
        const primaryKey = this.primaryKey || this.static.primaryKey;

        if (!this.exists) return this.toJson();

        const changed = {};
        changed[primaryKey] = this[primaryKey];

        for (let i = 0; i < this.#changed.length; i++) {
            const prop = this.#changed[i];
            if (!schema[prop]) continue;
            changed[prop] = this[prop];
        }
        ...
        if (only) {
            for (const prop in schema) {
                if (only.includes(prop)) {
                    if (schema[prop]?.type && schema[prop].type == "collection") {
    ...
    async #initializeRelationship(relationType, key, value, schema) {
        ...
        Object.defineProperty(this, key, {
            get() {
                //Allow Lazy Load
                if (!relation && typeof loadRelationship === "function") {
                    loadRelationship();
                }
            ...
            set(data) {
                ...
                if (!relation) {
                    if (relationType === "hasMany") {
                        relation = RelatedModel.Collection([]);
                        relation.setParent(this);
                    } else {
                        relation = new RelatedModel({}, { parent: this, with: [] });
                        relation[foreignKey] = this[localKey];
                    }
                    attachEvents();
                }
                ...
                if (relationType === "hasMany") {
                    ...
                    } else if (data && data.isCollection && typeof data.toJson === "function") {
                        relation.update(data.toJson());
                    } else if (data instanceof RelatedModel || type(data, "object")) {
                        relation.push(data);
                    } else {
                        throw new Error(
                            `Trying to add value of ${typeof data} to relation when ${RelatedModel.name} data is required`
                        );
                    }
                } else {
                    if (data instanceof RelatedModel) {
                        relation.reset(data.toJson());
