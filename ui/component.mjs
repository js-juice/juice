    ...
    return {
        [name]: class extends BaseHTMLElement {
            ...
            static initialize() {
                ...
                const userConfig = getJuiceConfig(`ui.components.${this.tag}`) || {};
            ...
            static renderProxy = {
                get: function (customElement, prop, receiver) {
                    const value = customElement[prop];
                    return value ?? "";
                }
            };
            ...
            constructor(contents = null) {
                ...
                if (this.debug) console.log("Created", this.constructor.name);
            ...
            #resize(width, height) {
                ...
                if (this.debug) console.log("resize", width, height);
                switch (resizeAction) {
                    case "width":
                        this.width = width;
                        break;
            ...
            _onFirstConnect() {
                ...
                if (type(this.onResize, "function") || this.RESIZE_ACTION !== "none") {
                    if (this._resizeMutationObserver) this._resizeMutationObserver.disconnect();
                    this._resizeMutationObserver = new MutationObserver(() =>
                        this.#resize(this.offsetWidth, this.offsetHeight)
                    );
                    this._resizeMutationObserver.observe(this, {
                        attributes: true
                    });
                }
                ...
                if (type(this.onPosition, "function")) {
                    if (this._positionMutationObserver) this._positionMutationObserver.disconnect();
                    this._positionMutationObserver = new MutationObserver(() => {
                        const x = this.offsetLeft;
                        const y = this.offsetTop;
                        this.onPosition(x, y);
                    });
                    this._positionMutationObserver.observe(this, {
                        attributes: true,
                        childList: true,
                        subtree: true
                    });
                ...
                if (this.onChildren) {
                    if (this._childrenMutationObserver) this._childrenMutationObserver.disconnect();
                    this._childrenMutationObserver = new MutationObserver((mutations) => {
                        const added = [];
                    ...
                    this._childrenMutationObserver.observe(this, {
                        childList: true
                    });
            ...
            disconnectedCallback() {
                ...
                delete CUSTOM_INSTANCES[this._id];
                if (this._resizeMutationObserver) {
                    this._resizeMutationObserver.disconnect();
                    this._resizeMutationObserver = null;
                }
                if (this._positionMutationObserver) {
                    this._positionMutationObserver.disconnect();
                    this._positionMutationObserver = null;
                }
            ...
            */
            ...
            _bindEvent(eventName, element, handlerName) {
                const openIndex = handlerName.indexOf("(");
                const closeIndex = handlerName.lastIndexOf(")");
                const methodName = (openIndex === -1 ? handlerName : handlerName.slice(0, openIndex)).trim();
                const argsText =
                    openIndex === -1 || closeIndex <= openIndex ? "" : handlerName.slice(openIndex + 1, closeIndex);
                const handlerFn = this[methodName] || (() => {});
                const args = argsText
                    .split(",")
